...
module.exports = {
	...
	activate() {
		...
		this.subscriptions.add(atom.commands.add('atom-workspace', {
			'typespess-editor:reload-env': this.reload_env.bind(this)
		}));
	...
	reload_env() {
		...
		var envfile_path = path.join(root_dir,".ts-env.json");
		if(!fs.existsSync(envfile_path))
			throw new Error("Environment file does not exist! You must create a .ts-env.json file.");
		var env_paths = JSON.parse(fs.readFileSync(envfile_path, "utf8"));
		global.workspaceDir = root_dir;
		global.workspaceDir.replace(`\\`,"/");
		global.workspaceDir += "/"
