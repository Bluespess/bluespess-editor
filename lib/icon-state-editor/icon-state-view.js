...
export default class IconStateView {
	...
	update_current_meta() {
		...
		for(var i = 0; i < dir_meta.frames.length; i++) {
			...
			this.keyframes += `${100*(time/total_delay)}% {background-position: ${-frame_meta.x}px ${-frame_meta.y}px}`
			time += frame_meta.delay;
		}
		this.animate = `animation: ${total_delay/1000}s step-end infinite ${this.idstart}anim`;
	}
}
