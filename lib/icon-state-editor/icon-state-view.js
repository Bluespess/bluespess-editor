...
export default class IconStateView {
	...
	render() {
		return (
			<div className={this.idstart + " " + this.properties.class + " " + this.properties.className} attributes={{style: this.properties.style}}>
				<style>{`
	...
	update_current_meta() {
		...
		if(!icon_state_meta)
			icon_state_meta = this.properties.meta[" "];
		if(!icon_state_meta)
			icon_state_meta = this.properties.meta[""];
		if(!icon_state_meta) {
			return;
		...
		if(!progression)
			progression = dir_progressions[icon_state_meta.dirCount];
		if(!progression)
		...
		if(dir_meta.length <= 1 || this.properties.frame != undefined) {
			return;
		}
		var totalDelay = 0;
		for(let i = 0; i < dir_meta.length; i++) {
			totalDelay += dir_meta[i].delay;
		}
		var time = 0;
		for(var i = 0; i < dir_meta.length; i++) {
			frame_meta = dir_meta[i];
			this.keyframes += `${100*(time/totalDelay)}% {background-position: ${-frame_meta.x}px ${-frame_meta.y}px}`
			time += frame_meta.delay;
		}
		this.animate = `animation: ${totalDelay/1000}s step-end infinite ${this.idstart}anim`;
	}
}
