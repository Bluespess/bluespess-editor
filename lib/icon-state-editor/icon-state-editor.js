...
const IconStateEditorView = require("./icon-state-editor-view");

// Editor model for an image file
class IconStateEditor {
  ...
  updateMetaFile() {
    fs.readFile(this.metaFile.getPath(), (err, data) => {
      ...
      if (this.editorView) this.editorView.metaFileChanged();
    });
  }
  ...
  serialize() {
    return {
      ...
      metaFilePath: this.metaFile.getPath(),
    };
  }
...
module.exports = IconStateEditor;
