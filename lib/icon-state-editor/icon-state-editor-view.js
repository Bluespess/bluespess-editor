...
module.exports = class IconStateEditorView {
  ...
  getStateViews() {
    var list = [];
    if (!this.editor.iconMeta) return list;
    for (let key in this.editor.iconMeta) {
      if (!this.editor.iconMeta.hasOwnProperty(key)) continue;
      list.push(`
        <div
          className="icon-state-list-item"
          title={key}
          onclick={this.selectIconState.bind(this, key)}
        >
          <IconStateView
            icon={this.refs && this.refs.image && this.refs.image.src}
            meta={this.editor.iconMeta}
            icon_state={key}
            className="icon-state-img"
          />
          <div
            className="icon-state-label"
            style={{
              width: Math.max(64, this.editor.iconMeta[key].width) + "px",
            }}
          >
            {key}
          </div>
      ...
      <div className="image-view bluespess-editor" tabIndex="-1">
        ...
        <div ref="imagepage">
          <div className="image-controls" ref="imageControls">
            ...
            <div className="image-controls-group btn-group">
              <button className="btn" ref="zoomToFitButton">
                Zoom to fit
              </button>
            </div>
          </div>
