...
export default class BSMap {
	...
	sort_objects() {
		...
		this.objects.sort(this.context.client_env.atom_comparator);
	}
}
...
class Instance {
	...
	set_pos(x, y) {
		...
		if(this.x != null && this.y != null) {
			...
			this.grid_tile.sort(this.map.context.client_env.atom_comparator);
		}
	...
	update_context() {
		this.client = this.map.context.client_env;
		var new_template = this.map.context.server_env.templates[this.template_name];
		if(new_template === this.base_template)
			return;
		...
		this._appearance_vars = this.computed_vars.appearance;
		// For those that don't understand: || returns the first var if it's equivalent to true, otherwise it returns the second var.
		// So basically (a ? a : b) is equivalent to (a || b)
		this.appearance_controller = new this.map.context.client_env.appearance_controllers[this._appearance_vars.appearance_controller_type || "Default"](this);
		this.on_appearance_change(Object.assign({}, this._appearance_vars));
	...
	on_appearance_change(changes) {
		...
		}*/ // piss off no overlays in the map editor. Too much work. someone else do it
	}
