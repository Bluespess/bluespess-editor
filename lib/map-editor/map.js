...
class BSMap {
    ...
    var data = stable_stringify(obj, {
      ...
      cmp: (a, b) => {
        ...
        if (a_coords && b_coords) {
          return (
            +a_coords[3] - +b_coords[3] ||
            +a_coords[2] - +b_coords[2] ||
            +a_coords[1] - +b_coords[1]
          );
        }
  ...
  sort_objects() {
    ...
    this.objects.sort((a, b) => {
      return this.context.client_env.constructor.Atom.atom_comparator(
        a.client_atom,
        b.client_atom
      );
    });
...
class Instance {
  constructor(map, instobj) {
    var x = instobj.x,
      y = instobj.y;
    if (x !== +x || y !== +y)
      // checks to make sure they're numbers and not NaN
  ...
  set_pos(x, y) {
    if ((x != null || y != null) && (x !== +x || y !== +y))
      // checks to make sure they're numbers and not NaN
      throw new TypeError(`Invalid coordinates: (${x},${y})`);
    if (this.grid_tile) {
      var idx;
      (idx = this.grid_tile.indexOf(this)) != -1 &&
        this.grid_tile.splice(idx, 1);
    }
    this.x = x;
    this.y = y;
    if (this.x != null && this.y != null) {
      ...
      this.grid_tile.sort((a, b) => {
        return this.map.context.client_env.constructor.Atom.atom_comparator(
          a.client_atom,
          b.client_atom
        );
      });
  ...
  update_context(force = false) {
    var new_template = this.map.context.server_env.templates[
      this.template_name
    ];
    if (new_template === this.base_template && !force) return;
    ...
    if (this.base_template.variants && this.base_template.variants.length) {
      if (!this.variant_leaf_path) this.variant_leaf_path = [];
      ...
      for (var i = 0; i < this.base_template.variants.length; i++) {
        ...
        if (variant.type == "single") {
          ...
          for (var j = 0; j < variant.var_path.length - 1; j++) {
            if (j == 0 && variant.var_path[j] == "appearance") continue;
    ...
    var client_instobj = {
      components: [],
      component_vars: this.computed_vars.components || {},
    };
    for (var key of [
      "icon",
      "icon_state",
    ...
    this.client_atom = new this.map.context.client_env.constructor.Atom(
      this.map.context.client_env,
      client_instobj
    );

    if (this.base_template.components)
      for (let component_name of this.base_template.components) {
        let component_class = this.map.context.server_env.components[
          component_name
        ];
  ...
  finalize_movement() {
    ...
    if (this.last_x != null && this.last_y != null) {
      let last_tile =
        this.map.grid[`${this.last_x},${this.last_y}`] ||
        (this.map.grid[`${this.last_x},${this.last_y}`] = []);
      for (let obj of last_tile) {
        if (obj == this) continue;
        if (obj.base_template.requires_under) {
          if (
            (obj.base_template.requires_under.component &&
              this.base_template.components.indexOf(
                obj.base_template.requires_under.component
              ) == -1) ||
            ...
            if (obj.base_template.requires_under.default) {
              new Instance(this.map, {
                template_name: obj.base_template.requires_under.default,
                x: obj.x,
                y: obj.y,
              }).finalize_movement();
            } else {
    ...
    if (this.x != null && this.y != null) {
      ...
      if (this.base_template.requires_under) {
        ...
        for (let obj of this.grid_tile) {
          if (
            (this.base_template.requires_under.component &&
              obj.base_template.components.includes(
                this.base_template.requires_under.component
              )) ||
        ...
        if (!success) {
          if (this.base_template.requires_under.default) {
            new Instance(this.map, {
              template_name: this.base_template.requires_under.default,
              x: this.x,
              y: this.y,
            }).finalize_movement();
          } else {
  ...
  del() {
    ...
    if (idx != -1) this.map.objects.splice(idx, 1);
  }
}
