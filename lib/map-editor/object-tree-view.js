		...
		for(var template_name in this.context.server_env.templates) {
			...
			for(var tree_path of template.tree_paths) {
				...
				for(var node_name of tree_path_split) {
					...
					let node = (last_node ? last_node.children_map[node_name] : this.tree_map[node_name]);
					if(!node) {
...
class ObjectTreeNode {
	...
	createElement() {
		...
		if(this.children.length) {
			...
			if(!this.template)
				this.name_span.classList.add("icon-file-directory");
		...
		if(this.template) {
			this.view.context.create_thumbnail({template_name: this.template_name}).then((thumbnail) => {
