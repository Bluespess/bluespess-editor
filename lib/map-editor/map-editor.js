...
class MapEditor {
  constructor(filepath, context) {
    ...
    this.disposables.add(
      new Disposable(() => {
        this.mapcanvas.removeEventListener("mousedown", this.canvas_mousedown);
      })
    );
    ...
    this.disposables.add(
      new Disposable(() => {
        this.mapcanvas.removeEventListener("wheel", this.canvas_wheel);
      })
    );
    ...
    this.disposables.add(
      new Disposable(() => {
        this.mapcanvas.removeEventListener("mousemove", this.canvas_mousemove);
      })
    );
    ...
    this.disposables.add(
      new Disposable(() => {
        this.mapcanvas.removeEventListener("mouseout", this.canvas_mouseout);
      })
    );
    ...
    this.disposables.add(
      new Disposable(() => {
        this.mapcanvas.removeEventListener(
          "contextmenu",
          this.canvas_contextmenu
        );
      })
    );
  ...
  draw(timestamp) {
    ...
    ctx.translate(
      Math.round(this.mapwindow_x * 32 * this.mapwindow_zoom) /
        this.mapwindow_zoom,
      -Math.round(this.mapwindow_y * 32 * this.mapwindow_zoom) /
        this.mapwindow_zoom
    );
    ctx.imageSmoothingEnabled = false;
    if (this.map) {
      if (this.map.needs_sort) this.map.sort_objects();
    ...
    if (this.mouse_tile_x !== undefined && this.mouse_tile_y !== undefined) {
      ...
      ctx.strokeRect(
        Math.floor(this.mouse_tile_x) * 32 + 0.5,
        -Math.floor(this.mouse_tile_y) * 32 + 0.5,
        31,
        31
      );
      ...
      ctx.globalAlpha = 0.5;
      ctx.strokeRect(
        Math.floor(this.mouse_tile_x) * 32 + 1.5,
        -Math.floor(this.mouse_tile_y) * 32 + 1.5,
        29,
        29
      );
      ctx.globalAlpha = 1;
  ...
  canvas_mousedown(e) {
    ...
    if (
      e.button == 1 ||
      (e.button == 0 && e.altKey && !e.ctrlKey && !e.shiftKey)
    ) {
      ...
      var mousemove = (e) => {
        this.mapwindow_x +=
          ((e.screenX - lastE.screenX) / 32 / this.mapwindow_zoom) *
          devicePixelRatio;
        this.mapwindow_y -=
          ((e.screenY - lastE.screenY) / 32 / this.mapwindow_zoom) *
          devicePixelRatio;
        lastE = e;
      };
      ...
      if (template_name) {
        var inst = new BSMap.Instance(this.map, {
          template_name,
          variant_leaf_path: this.getActiveVariantPath(),
          x: Math.floor(this.mouse_tile_x),
          y: Math.floor(this.mouse_tile_y),
        });
  ...
  canvas_contextmenu(e) {
    this.update_mouse_tile(e);
    if (this.contextmenu_disposable) this.contextmenu_disposable.dispose();
    if (this.contextmenu_actions_disposable)
      this.contextmenu_actions_disposable.dispose();
    var num = 0;
    var menu_items = [];
    var actions = {};
    var tile = this.map.grid[
      `${Math.floor(this.mouse_tile_x)},${Math.floor(this.mouse_tile_y)}`
    ];
    if (!tile) return;
    for (let i = tile.length - 1; i >= 0; i--) {
      let item = tile[i];
      var icon = item.thumbnail
        ? electron.remote.nativeImage.createFromDataURL(item.thumbnail.data)
        : null;
      let menu_item = {
        label: `${item.computed_vars.name} (${item.template_name})${Array(
          ++num
        ).join(" ")}`,
        icon,
      };
    ...
    this.contextmenu_disposable = atom.contextMenu.add({
      ".mapcanvas": menu_items,
    });
    this.contextmenu_actions_disposable = atom.commands.add(
      "atom-workspace",
      actions
    );
  }
  ...
  update_mouse_tile(e) {
    this.mouse_tile_x =
      (e.offsetX * devicePixelRatio - this.mapcanvas.width / 2) /
        this.mapwindow_zoom /
        32 -
      this.mapwindow_x;
    this.mouse_tile_y =
      -(e.offsetY * devicePixelRatio - this.mapcanvas.height / 2) /
        this.mapwindow_zoom /
        32 -
      this.mapwindow_y +
      1;
  ...
  canvas_wheel(e) {
    ...
    this.mapwindow_log_zoom = Math.max(
      -5,
      Math.min(5, this.mapwindow_log_zoom)
    );
  ...
  getActiveTemplateName() {
    return (
      this.context.object_tree_view &&
      this.context.object_tree_view.selected &&
      this.context.object_tree_view.selected.template_name
    );
  }
