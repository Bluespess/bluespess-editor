				...
				for(var i = 0; i < variant.values.length; i++) {
					...
					try {
						...
						if(variant.label) {
							...
							if(variant.var_path.length == 1 && variant.var_path[0] == "dir" && typeof variant.values[i] == "number") {
								variant_label = "";
								if(variant.values[i] & 1)
									variant_label += "north";
								if(variant.values[i] & 2)
									variant_label += "south";
								if(variant.values[i] & 4)
									variant_label += "east";
								if(variant.values[i] & 8)
									variant_label += "west";
							}
							var textnode = document.createTextNode((variant.label_prefix || "") + variant_label + (variant.label_suffix || ""));
