...
module.exports = class VariantView {
  constructor(context) {
    ...
    this.disposables.add(
      this.context.showObjectTreeView().onSelectionChanged(this.build_variants)
    );
  }
  destroy_variants() {
  ...
  async build_variants() {
    ...
    try {
      ...
      this.element.appendChild(
        await this.build_variants_for_path([], template, template_name)
      );
    } catch (e) {
      console.error(e);
    }
  ...
  async build_variants_for_path(path, template, template_name) {
    ...
    if (!template.variants || path.length >= template.variants.length) {
      ...
      var thumbnail = await this.context.create_thumbnail({
        template_name,
        variant_leaf_path: path,
      });
      ...
      if (variant.type == "single") {
        for (var i = 0; i < variant.values.length; i++) {
          ...
          try {
            var variant_element = await this.build_variants_for_path(
              new_path,
              template,
              template_name
            );
            ...
            if (variant.label) {
              let variant_label = variant.values[i];
              if (
                variant.var_path.length == 1 &&
                variant.var_path[0] == "dir" &&
                typeof variant.values[i] == "number"
              ) {
                variant_label = "";
                if (variant.values[i] & 1) variant_label += "north";
                if (variant.values[i] & 2) variant_label += "south";
                if (variant.values[i] & 4) variant_label += "east";
                if (variant.values[i] & 8) variant_label += "west";
              }
              var textnode = document.createTextNode(
                (variant.label_prefix || "") +
                  variant_label +
                  (variant.label_suffix || "")
              );
              if (variant.put_label_before) {
                variant_element_container.insertBefore(
                  textnode,
                  variant_element
                );
              } else {
            ...
            if (variant.orientation == "vertical") {
              variant_element_container.classList.add(
