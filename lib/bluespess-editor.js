...
module.exports = {
  ...
  activate() {
    ...
    this.subscriptions.add(
      atom.workspace
        .getCenter()
        .onDidChangeActivePaneItem(this.attachMapEditorStatusView)
    );
    this.subscriptions.add(
  ...
  deactivate() {
    ...
    if (this.object_tree_view) this.object_tree_view.destroy();
  ...
  attachMapEditorStatusView() {
    if (this.status_view_attached) return;
    if (this.status_bar == null) return;
  ...
  reload_env() {
    ...
    if (!fs.existsSync(envfile_path))
      throw new Error(
        "Environment file does not exist! You must create a .bs-env.json file."
      );
    ...
    var keep_cr_keys = global.snapshotResult
      ? new Set(Object.keys(global.snapshotResult.customRequire.cache))
      : null;
    // Load the environments
    console.log(
      path.join(root_dir, env_paths.server_env_path) +
        ", " +
        path.join(root_dir, env_paths.client_env_path)
    );
  ...
  openURI(uri) {
    ...
    if (extname == ".png") {
      ...
      if (!fs.existsSync(metauri)) return;
      return new IconStateEditor(uri, metauri);
    }
    if (extname == ".bsmap") {
      if (!this.server_env) this.reload_env();
  ...
  showObjectTreeView() {
    if (!this.object_tree_view) {
      ...
      this.object_tree_view.onDidDestroy(() => {
        this.object_tree_view = null;
      });
      if (this.server_env) this.object_tree_view.buildTree();
  ...
  showVariantView() {
    if (!this.variant_view) {
      ...
      this.variant_view.onDidDestroy(() => {
        this.variant_view = null;
      });
    }
  ...
  async create_thumbnail(instobj) {
    ...
    if (!base_template)
      return Promise.resolve({ width: 0, height: 0, data: "" });
    if (!base_template.thumbnails) base_template.thumbnails = {};
