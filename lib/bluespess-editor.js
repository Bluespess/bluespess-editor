...
import MapEditorStatusView from './map-editor/map-editor-status-view';
import BSMap from './map-editor/map';
import ObjectTreeView from './map-editor/object-tree-view';
import VariantView from './map-editor/variant-view';
...
export default {
	...
	activate() {
		...
		this.attachImageEditorStatusView = this.attachImageEditorStatusView.bind(this);
		this.subscriptions.add(atom.workspace.getCenter().onDidChangeActivePaneItem(this.attachImageEditorStatusView));
	...
	async create_thumbnail(instobj) {
		...
		var key = instobj.variant_leaf_path ? JSON.stringify(instobj.variant_leaf_path) : "";
		...
		var bounds = instance.appearance_controller.get_bounds(0);
		if(!bounds) {
			await instance.appearance_controller.fully_load();
			bounds = instance.appearance_controller.get_bounds(0);
			if(!bounds)
		...
		instance.appearance_controller.on_render_tick(0);
		instance.appearance_controller.draw(ctx, 0);
		instance.del();
